function checkForm(form) {
	...
	/*
	 * if(form.email.value == ""){ alert("Error:Email cannot be blank!");
	 * form.email.focus(); return false; }
	 * 
	 * re = "/^w+@\\.w+{2,3}$"; if(!re.test(form.email.value) ||
	 * (form.email.value.length >30)) { alert("Error: Email doesn't match the
	 * pattern or it's too long. Email should contain less then 30 simbols!");
	 * form.email.focus(); return false; }
	 */
	re = /[a-zA-Z]/;
	...
	if ((form.password1.value != "")
		...
		if ((form.password1.value.length < 6)
			...
			alert("Error: Password must contain at least six characters and no more than 30!");
